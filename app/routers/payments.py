...
from app.schemas import PaymentIntentCreate, PaymentIntentResponse, PaymentWebhook, PaymentStatus
from app.deps import get_current_user, get_rate_limiter
from app.utils import log_request, log_response, generate_correlation_id, generate_provider_ref
...
def payment_webhook(
    ...
    try:
        ...
        payment_attempt = db.query(PaymentAttempt).filter(
            PaymentAttempt.provider_ref == webhook_data.provider_ref
        ).first()
